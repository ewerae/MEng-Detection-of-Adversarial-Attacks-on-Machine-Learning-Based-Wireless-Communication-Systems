...
""" README: THIS PARTICULAR FILE CREATES A UAP BASED ON ONLY ICM INPUTS AS MENTIONED IN THE REPORT
THIS SECTION FIRST IMPORTS THE INDEXES WHERE THE NEURAL NETWORK CAN CORRECTLY CLASSIFY (CCM) 
AND THEN REMOVES IT FROM ALL POSSIBLE INDEXES. IT ALSO REMOVES THE TRAINING INDEXES.

IT FIRST CREATES A LIST OF ALL POSSIBLE 10SNR INDEXES OF ALL MODULATIONS AND THEN REMOVES CCM INPUTS AND NN TRAINED INPUTS
"""
...
"""
train_Index, test_Index = train_test_split(all_numbers, test_size=0.5, random_state=42)
...
In the report, we create a UAP based on CCM, ICM and MIX data inputs. This SPECIFIC file creates a UAP based on ICM. 
...
"""
# # ================================================ 1. APPLYING ICM UAP TO CCM =================================================== #
# """1. PLEASE UNCOMMENT THIS SECTION TO PRODUCE THE SOFTMAX OUTPUT FOR CCM DATA INPUTS THAT ARE AFFECTED BY THE ICM BASED UAP """
#
# random_numbers = []
...
#
# file_Name_Normal = f"{PSR}PSR {SNR}SNR ICM CCM Normal.txt"
#
...
#
# file_Name_Adverse = f"{PSR}PSR {SNR}SNR ICM CCM Adverse.txt"
...
# ================================================ 2. APPLYING ICM UAP to ICM DATA INPUTS =========================================#
"""2. PLEASE UNCOMMENT THIS SECTION TO PRODUCE THE SOFTMAX OUTPUT FOR ICM DATA INPUTS THAT ARE AFFECTED BY THE ICM BASED UAP """
...
# """ Stores in text files which can be used immediately in the metrics and classifiers"""
#
# file_Name_Normal = f"{PSR}PSR {SNR}SNR ICM ICM Normal.txt"
#
# f = open(file_Name_Normal, "w")
# for i in range(length):
#     for n in range(11):
...
#         print(softmax_Output_Normal[i][0][n])
#
# file_Name_Adverse = f"{PSR}PSR {SNR}SNR ICM ICM Adverse.txt"
# f = open(file_Name_Adverse, "w")
# for i in range(length):
#     for n in range(11):
#         f.write(f"{softmax_Output_Attack[i][0][n]}\n")
...
# # =================================================3. APPLYING ICM UAP TO MIX =================================#
#"""3. PLEASE UNCOMMENT THIS SECTION TO PRODUCE THE SOFTMAX OUTPUT FOR MIX DATA INPUTS THAT ARE AFFECTED BY THE ICM BASED UAP """
...
file_Name_Normal = f"{PSR}PSR {SNR}SNR ICM MIX Normal.txt"
...
file_Name_Adverse = f"{PSR}PSR {SNR}SNR ICM MIX Adverse.txt"
f = open(file_Name_Adverse, "w")
for i in range(length):
    for n in range(11):
        f.write(f"{softmax_Output_Attack[i][0][n]}\n")
